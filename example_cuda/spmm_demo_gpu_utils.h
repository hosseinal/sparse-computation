...
public:
  ...
  ~SpMMcuBlas() {
    ...
    __device__ void PatternWith4NNZByRowPanel4RowPerThread2BlockCacheLess(
...
int number_of_row_panel_packs = (rowEnd_all - rowStart_all + 2 - 1) / 2;
int row_panel_pack_share = ((number_of_row_panel_packs + gridDim.y - 1) / gridDim.y) * 2;
...
float sum_2 = 0.0f;
float sum_3 = 0.0f;
for (int j = row_panel_start; j < row_panel_end; j += 2) {
...
sum_0 += nnz0_0 * b0 + nnz0_1 * b1 ;
sum_1 += nnz1_0 * b0 + nnz1_1 * b1 ;
sum_2 += nnz2_0 * b0 + nnz2_1 * b1 ;
sum_3 += nnz3_0 * b0 + nnz3_1 * b1 ;
t_nnz += 8;
}
int c_row = i * 4;
...
__device__ void PatternWith4NNZByRowPanel3RowPerThread2BlockCacheLess(
...
__device__ void PatternWith4NNZByRowPanel2RowPerThread2BlockCacheLess(
...
__device__ void PatternWith4NNZByRowPanel1RowPerThread2BlockCacheLess(
...
__global__ void
SpMMFullRowPanelGeneralKernel4With2BlockCacheLess(swiftware::compression::MixedFormatGPU<float> *mf, float *dX, float *dY, int c_cols) {
  int i = blockIdx.x;
  int pattern = i % 15;
  switch (pattern) {
    case 0:
      PatternWith4NNZByRowPanel4RowPerThread2BlockCacheLess(
          mf->cf[0]->RPP, mf->cf[0]->NPP, mf->cf[0]->NNZ, mf->cf[0]->CB, dX, dY,
          i / 15, c_cols , 0 , 1 , 2 , 3);
      break;
    case 1:
      PatternWith4NNZByRowPanel3RowPerThread2BlockCacheLess(
    ...
    case 2:
      ...
      break;
    case 3:
      PatternWith4NNZByRowPanel2RowPerThread2BlockCacheLess(
    ...
    case 6:
      ...
      break;
    case 7:
      PatternWith4NNZByRowPanel1RowPerThread2BlockCacheLess(
...
__global__ void
SingleSpMMFullRowPanelGeneralKernel4With2BlockCacheLess(swiftware::compression::MixedFormatGPU<float> *mf, float *dX, float *dY, int c_cols) {
  int i = blockIdx.x;
  int pattern = i % 15;
  PatternWith4NNZByRowPanel4RowPerThread2BlockCacheLess(
          mf->cf[0]->RPP, mf->cf[0]->NPP, mf->cf[0]->NNZ, mf->cf[0]->CB, dX, dY,
...
SpMMFullRowPanelGeneralKernel4With2BlockCacheLess<<<gridDim,
32>>>(mf, dX, dY, InTensor->Z);
